...
public class IntegerHandler implements IValueHandler<Integer> {
	...
	public void uncheckedEdit(YamlEditor yfe, ValueEditor<?> parent, String key, List<IValueHandler<?>> valHandler, UUID id, String title,
		...
		IValueHandler.super.uncheckedEdit(yfe, parent, key, valHandler, id, title, () -> ((Number) valSup.get()).intValue(), onEdit, onBack,
				onDelete);
	}
	...
	public void edit(YamlEditor yfe, ValueEditor<?> parent, String key, List<IValueHandler<?>> valHandler, UUID id, String title,
			Supplier<Integer> valSup, Consumer<Integer> onEdit, Runnable onBack, Runnable onDelete) {
		LongEditor le = new LongEditor(id, title, parent, yfe, key, () -> valSup.get().longValue(), l -> onEdit.accept(l.intValue()), onBack,
				onDelete);
