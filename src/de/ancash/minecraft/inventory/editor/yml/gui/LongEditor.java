...
public class LongEditor extends ValueEditor<Long> {
	...
	public LongEditor(UUID id, String title, ValueEditor<?> parent, YamlEditor yeditor, String key, Supplier<Long> valSup, Consumer<Long> onEdit,
	...
	public void acceptInput() {
		NumberInputGUI<Long> nig = new NumberInputGUI<>(ILibrary.getInstance(), Bukkit.getPlayer(getId()), Long.class, s -> {
			onEdit.accept(s);
			Bukkit.getScheduler().runTaskLater(ILibrary.getInstance(),
					() -> new LongEditor(getId(), title, parent, yeditor, key, valSup, onEdit, onBack, onDelete), 1);
		}, l -> {
