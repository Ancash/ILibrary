...
public class ItemStackFileUtil {
	...
	static {
		...
		GET_CONSUMERS.put("meta.lore", (item, lore) -> ItemStackUtils.setLore(((List<String>) lore).stream().map(s -> ChatColor.translateAlternateColorCodes('&', s)).collect(Collectors.toList()), item));
		GET_CONSUMERS.put("meta.enchantments", (item, enchantments) -> ((List<String>) enchantments).forEach(ench -> item.addUnsafeEnchantment(Enchantment.getByName(ench.split(":")[0]), Integer.valueOf(ench.split(":")[1]))));
		GET_CONSUMERS.put("meta.flags", (item, flags) -> ((List<String>) flags).stream().map(ItemFlag::valueOf).forEach(flag -> ItemStackUtils.addItemFlag(item, flag)));
	}
	...
	public static ItemStack getItemStack(FileConfiguration fc, String path) {
		...
		if(cs.contains("custom-model-data")) 
			try {
				...
				System.err.println("Could not set custom model data " + cs.getInt("custom-model-data") + " for " + path);
			}
		
		for(String prop : cs.getKeys(true)) {
			if(cs.isConfigurationSection(prop)) continue;
